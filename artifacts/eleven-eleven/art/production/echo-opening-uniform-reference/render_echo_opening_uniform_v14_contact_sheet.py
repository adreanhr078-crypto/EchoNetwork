"""Render isolated Rokoko-retarget v14 phase and angle review images."""
...
camera_data = bpy.data.cameras.new("V14GaitReviewCamera")
camera = bpy.data.objects.new("V14GaitReviewCamera", camera_data)
...
for row, (action_name, cycle) in enumerate((("preset:walk", 34), ("preset:run", 20))):
    for sample in range(6):
        render_pose(action_name, sample / 6.0, cycle, REF / f"v14-phase-frame-{row * 6 + sample:02d}.png")
...
for row, (action_name, phase, cycle) in enumerate((
    ...
    for angle_index in range(8):
        ...
        render_pose(action_name, phase, cycle, REF / f"v14-angle-frame-{row * 8 + angle_index:02d}.png")

print("V14_REVIEW_FRAMES", 36)
