...
bpy.ops.wm.open_mainfile(filepath=str(root/'art/production/echo-opening-uniform-reference/echo-opening-uniform-v7.blend'))
scene=bpy.context.scene;rig=bpy.data.objects['EchoOpeningUniformRig']
for t in rig.animation_data.nla_tracks:t.mute=True
action=bpy.data.actions['preset:run'];rig.animation_data.action=action
...
for fr in range(1,32):
 scene.frame_set(fr)
 phase=2*math.pi*(fr-1)/15
 ...
 for side,sign in [('Left',1.0),('Right',-1.0)]:
  ...
  hip.rotation_quaternion=Quaternion((1,0,0),0.72*local_swing)
  hip.keyframe_insert(data_path='rotation_quaternion',frame=fr,group=hip.name)
  knee.rotation_quaternion=Quaternion((1,0,0),0.78*max(0.0,local_swing))
  knee.keyframe_insert(data_path='rotation_quaternion',frame=fr,group=knee.name)
  foot.rotation_quaternion=Quaternion((1,0,0),-0.22*local_swing-0.15*max(0.0,local_swing))
...
blend=root/'art/production/echo-opening-uniform-reference/echo-opening-uniform-v8.blend'
out=root/'godot/assets/characters/echo_opening_uniform_v8.glb'
...
print('OUTPUT',out,'RUN_FRAMES',action.frame_range)
