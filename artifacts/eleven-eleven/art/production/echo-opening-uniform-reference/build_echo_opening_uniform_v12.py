"""Build an upright Echo locomotion candidate on the v10 story rig.
...
BLEND = REF / "echo-opening-uniform-v12.blend"
GLB = ROOT / "godot/assets/characters/echo_opening_uniform_v12.glb"
FPS = 24
MODEL_SCALE = 1.81
...
def flight_envelope(phase, spec):
    if "flight_rise" not in spec:
        return 0.0
    half_phase = phase % 0.5
    if half_phase <= spec["stance"]:
        return 0.0
    flight_phase = (half_phase - spec["stance"]) / (0.5 - spec["stance"])
    return math.sin(math.pi * flight_phase)
...
def foot_goal(side, phase, spec):
    ...
    if side_phase < stance:
        ...
        lift += spec.get("flight_toe_lift", 0.0) * flight_envelope(phase, spec)
    toe_rest = armature.bones[f"tripo::1_{side}_Limb_3"].tail_local
    # Tripo's toe-bone tip is 0.052 rig metres below the shoe sole at idle.
    # The rest-height tip target therefore puts the skinned sole on the floor.
    return toe_rest + Vector((horizontal, 0.0, lift)), side_phase < stance
...
def solve_frame(source_pose, phase, spec, geometry):
    ...
    if "flight_rise" in spec:
        root_matrix.translation.z += -spec["pelvis_drop"] + spec["flight_rise"] * flight_envelope(phase, spec)
    else:
...
print("V12_GAIT_REPORT", reports)
print("V12_OUTPUT", GLB)
